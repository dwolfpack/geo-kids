...
| 13 | Sky Rescue is a firefighting and rescue helicopter, not a combat jet. Water bombs on wildfires replace guns and missiles. | This keeps After Burner's arcade verbs (rail speed, roll, targets, hit feedback) while staying right for ages 7–12. |
| 14 | three.js r169 is bundled into `games/sky-rescue/vendor/` (MIT, license included) and loaded as an ES module. | The game works offline as a PWA with no CDN. ES modules need http(s), so this game needs GitHub Pages or a local server and can't run from a double-clicked file. |
| 15 | Sky Rescue is built with the Gauntlet Loop. Fresh critic subagents with clean context judge real pixels (stills, filmstrips, full autopilot playthroughs) against `ref/REFERENCE.md`, and each round fixes their single biggest gap. | The user asked for "like this" (Dan Shamir's VECTOR BURN write-up). The reference image was generated with Canva and is described in REFERENCE.md, because the connector returns only a preview. |
